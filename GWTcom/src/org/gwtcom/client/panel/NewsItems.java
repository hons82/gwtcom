...
public class NewsItems {
	...
	private static final int NUM_ITEMS = 64;
	  ...
	  private static int senderIdx = 0, emailIdx = 0, subjectIdx = 0,
	      fragmentIdx = 0;
	  private static ArrayList<NewsItem> items = new ArrayList<NewsItem>();
	  ...
	  public static NewsItem getNewsItem(int index) {
	    if (index >= items.size()) {
	      return null;
	    }
	  ...
	  public static int getNewsItemCount() {
	    return items.size();
	  }

	  private static NewsItem createFakeMail() {
	    ...
	    return new NewsItem(sender, email, subject, body);
	  }
